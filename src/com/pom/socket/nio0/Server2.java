...
public class Server2 {
	...
	public void listen() throws IOException {
		while(running) {
			...
			System.out.println("selectorCount: "+ selectorCount);
			if(selectorCount > 0) {
				...
				while(iteratorKeys.hasNext()) {
					...
					if(key.isReadable()) {
						...
						System.out.println("In Readable... "+channel);
						StringBuilder requestSb = new StringBuilder();
						//使用前先清洁一下
						readBuffer.clear();
						if(channel.read(readBuffer) > 0) {
							readBuffer.flip();
							requestSb.append(new String(readBuffer.array() ,0 ,readBuffer.limit()));
							readBuffer.clear();
						}
